...
「粘贴到浏览器里就能打开」→「发送到微信聊天中就能打开」。
粘到聊天窗口点开就是微信内置浏览器，用户不用离开微信，这条路比切浏览器短。

### 2. 成长树：七个圈 → 七张图标

原来的树是几个 `border-radius: 50%` 的圈拼的，在 80rpx 的框里更像气泡不像树。

改成七张线稿图标（`images/tree/`），一个成长阶段一张：
种子 / 破土 / 抽枝 / 成叶 / 繁茂 / 结蕾 / 开花。

生成脚本留在 `tools/mktree.py`，要调形状直接改坐标重跑。
环境里没有 PIL / cairosvg / imagemagick，所以脚本自带 PNG 编码器和
SDF 抗锯齿栅格化 —— 纯标准库，无依赖。

几个画法上的决定，都是试出来的：

- **叶子的中脉不画满。** 椭圆用的是近似 SDF，中脉画到叶尖会戳出去，
  变成一根小尾巴。收到 0.44 倍半径。
- **顶芽要斜着长。** 水平的顶叶看起来像扣了个盘子。
- **繁茂之后不画单片叶子，收成一个完整树冠。** 这个语言切换是有意的：
  前四个阶段数得清叶子，从这里开始数不清了。
- **树冠里不加枝条。** 那两笔在小尺寸下像裂纹。
- **花苞和花挂在树冠轮廓上，不放冠内。** 放冠内的两个点会读成一双眼睛，
  整棵树变成一张脸。
- **主干正好顶在树冠下缘。** 多一点会在冠里露出个断头（像棒棒糖的棍），
  少一点会留缝；冠的描边宽度刚好盖住主干的圆头。

图标只有一种绿（`#6E9480`），深浅两个主题都读得清，不必备两套 ——
和 tabBar 图标同一个思路。
